/*
	...
	- settings
		...
		- before: функция срабатывает перед отправкой запроса
	...
	Для полей
	- rules
		- empty: не допустить пустое поле
...
$.fn.formSubmit = function(settings) {
	...
	$(formSelector).find('input[name], textarea[name], select[name], button[name], [contenteditable][name]').not('input[type="button"]').each(function() {
		if (settings.ignore) {
		...
		var f = this,
			...
			v = (t == 'file') ? ($(f).prop('files').length ? $(f).prop('files') : null) : (['checkbox', 'radio'].indexOf(t) != -1 ? ($(f).is(':checked') ? (t == 'radio' ? $(f).val() : 1) : (t == 'checkbox' ? null : 0)) : (t == 'contenteditable' ? (getContenteditable($(f)) || null) : ((t =='hidden' ? ($(f).prop('value') || null) : $(f).val()) || null)));
	...
	if (errorFields.length > 0) {
		...
		$.each(errorFields, function(k, item) {
			if ($(item.field).attr('type') == 'hidden') {
				console.error('formSubmit -> Ошибка в скрытом поле:', item.error, item.field);
				return true;
			} 
